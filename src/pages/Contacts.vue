<template>
  <div class="content-bg">
    <div class="concact content">
      <sidebar :title="title" :sidebarItems="sidebarItems"></sidebar>
      <div class="page-content-wrap">
        <bread-crumbs :path="path"></bread-crumbs>
        <p class="page-content" v-html="pageContent"></p>
      </div>
      <router-view></router-view>
    </div>
  </div>
</template>

<script>
  import axios from 'axios'
  import Sidebar from '../components/Sidebar'
  import BreadCrumbs from '../components/BreadCrumbs'
  import RouterConst from '../config/RouterConst'

  export default {
    components: {
      Sidebar,
      BreadCrumbs
    },
    data () {
      return {
        path: this.$route.path,
        indexApi: RouterConst.contacts.index.api,
        title: RouterConst.contacts.index.title,
        sidebarItems: [
          {title: RouterConst.contacts.contact.title, href: RouterConst.contacts.contact.href},
          {title: RouterConst.contacts.join.title, href: RouterConst.contacts.join.href},
          {title: RouterConst.contacts.careers.title, href: RouterConst.contacts.careers.href},
        ],
        pageContent: '',
        cache: {}
      }
    },
    created() {
      this.fetchData()
    },
    watch: {
      $route() {
        this.fetchData()
      }
    },
    methods: {
      fetchData () {
        window.scroll(0, 0)
        const path = this.$route.path
        const pathArr = path.split('/')
        const path1 = pathArr[1]
        const path2 = pathArr[2]
        let api
        if (path2) {
          api = RouterConst[path1][path2].api
        } else {
          api = RouterConst.about.index.api
        }
        if (this.cache[path]) {
          this.pageContent = this.cache[path]
          this.path = path
        } else {
          axios.get(api)
            .then((resp) => {
              this.pageContent = resp.data.value
              this.path = path
              this.cache[path] = resp.data.value
            })
        }
      }
    }
  }
</script>

<style lang="scss">
  @import '../sass/variables';
  .content-bg {
    overflow: hidden;
  }
  .concact {
   &.content {
      overflow: hidden;
      padding: 20px;
    }
    .page-content-wrap {
      width: 770px;
      float: right;
      display: inline-block;
      overflow: hidden;

      p {
        color: $color-content-title;
        font-size: 14px;
        line-height: 2.1;
      }
    }
  }

</style>